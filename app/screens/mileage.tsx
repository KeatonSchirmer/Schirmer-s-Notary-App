...
export default function Mileage() {
  ...
  useEffect(() => {
    async function fetchMileage() {
      ...
      setError("");
      try {
        const data = await apiRequest("/mileage/", "GET", null, { "X-User-Id": String(userId) });
  ...
  const stopTracking = async () => {
    ...
    if (distance > 0) {
      try {
        ...
        setEntries((prev) => [
          { id: Date.now(), date: new Date().toISOString().split("T")[0], miles: distance.toFixed(2), purpose: "Tracked Trip" },
          ...prev,
        ]);
      } catch (err) {
